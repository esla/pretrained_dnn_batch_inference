...
class Augmentations:
    def __init__(self, **augs):
        ...
        piecewise_affine = iaa.PiecewiseAffine(scale=(0.0, 0.1), nb_rows=4, nb_cols=4, mode='symmetric')

        if augs['random_crop']:
            tf_list.append(transforms.RandomResizedCrop(augs['size'], scale=(0.4, 1.0)))
        else:
        ...
        if augs['color_saturation'] or augs['color_contrast'] or augs['color_brightness'] or augs['color_hue']:
        ...
        self.tf_transform = transforms.Compose([self.tf_augment, transforms.Normalize(augs['mean'], augs['std'])
        ])
