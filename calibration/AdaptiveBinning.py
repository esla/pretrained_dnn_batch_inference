...
def AdaptiveBinning(infer_results, show_reliability_diagram=True):
    ...
    Arguments:
    infer_results (list of list): a list where each element "res" is a two-element list denoting the infer result of a
    single sample. res[0] is the confidence score r and res[1] is the correctness score c. Since c is either 1 or 0,
    here res[1] is True if the prediction is correct and False otherwise.
    show_reliability_diagram (boolean): a boolean value to denote whether to plot a Reliability Diagram.
    ...
    for i, confindence_correctness in enumerate(infer_results):
        ...
        #esla tmp
        first_bin.append(confidence_score)
